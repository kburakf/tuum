...
public class TransactionControllerImplIntegrationTest {
    ...
    private void createTestBalance(String accountId, Currency currency, BigDecimal amount) {
    ...
    public void should_create_transaction() {
        ...
        createTestBalance(accountId, Currency.USD, new BigDecimal("500"));
    ...
    public void should_not_create_transaction_with_invalid_account() {
        CreateTransactionRequest transactionRequest = new CreateTransactionRequest();
        transactionRequest.setAccountId("non-existing-id");
        transactionRequest.setAmount(new BigDecimal("50"));
        transactionRequest.setCurrency("USD");
        transactionRequest.setDirection("IN");
        transactionRequest.setDescription("Test Invalid Account");
    ...
    public void should_not_create_transaction_due_to_insufficient_funds() {
        ...
        createTestBalance(accountId, Currency.USD, new BigDecimal("50"));

        CreateTransactionRequest transactionRequest = new CreateTransactionRequest();
        transactionRequest.setAccountId(accountId);
        transactionRequest.setAmount(new BigDecimal("150"));
        transactionRequest.setCurrency("USD");
        transactionRequest.setDirection("OUT");
        transactionRequest.setDescription("Insufficient funds transaction");
    ...
    public void should_not_create_transaction_with_invalid_direction() {
        CreateTransactionRequest transactionRequest = new CreateTransactionRequest();
        transactionRequest.setAccountId("account-id");
        transactionRequest.setAmount(new BigDecimal("50"));
        transactionRequest.setCurrency("USD");
        transactionRequest.setDirection("INVALID_DIRECTION");
        transactionRequest.setDescription("Invalid direction transaction");
    ...
    public void should_throw_account_not_found_exception() {
        ...
        ResponseEntity<CreateTransactionResponse> response = restTemplate.postForEntity(transactionBaseUrl, request, CreateTransactionResponse.class);
    ...
    public void should_not_create_transaction_when_account_id_is_missing() {
        ...
        transactionRequest.setCurrency("USD");
        transactionRequest.setDirection("IN");
        transactionRequest.setDescription("Missing account ID transaction");
    ...
    public void should_not_create_transaction_with_negative_amount() {
        String accountId = createTestAccount("123", "EE");
        createTestBalance(accountId, Currency.USD, new BigDecimal("100"));
    ...
    public void should_not_get_transaction() {
        ResponseEntity<GetTransactionResponse> response = restTemplate.getForEntity(transactionBaseUrl + "/" + "123", GetTransactionResponse.class);

        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.BAD_REQUEST);
        assertThat(response.getBody()).isNotNull();
        assertThat(response.getBody().getErrorMessage()).isEqualTo(ConstantErrorMessages.TRANSACTION_NOT_FOUND);
    }
    ...
    public void should_get_two_transactions_for_account() {
        String accountId = createTestAccount("456", "US");
        createTestBalance(accountId, Currency.USD, new BigDecimal("300"));
        ...
        ResponseEntity<List<GetTransactionResponse>> response = restTemplate.exchange(
                ...
                HttpMethod.GET,
                null,
                new ParameterizedTypeReference<List<GetTransactionResponse>>() {});
